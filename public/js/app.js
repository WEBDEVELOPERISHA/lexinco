...
async function generatePDFBlob() {
    ...
    // Render HTML content directly to PDF with automatic pagination
    await doc.html(legalNoticeDiv, {
        x: 20,          // Left margin in mm
        y: 30,          // Top margin in mm
        width: 170,     // Content width (210mm page width - 2 * 20mm margins)
        windowWidth: 650 // Approximate pixel width for scaling
    });

    // Add footer only on the last page
    const pageCount = doc.internal.getNumberOfPages();
    doc.setPage(pageCount); // Switch to the last page
    doc.setFontSize(8);
    doc.setTextColor(150);
    doc.text(`Generated by lexinco.com - Page ${pageCount}`, 20, 287); // Near bottom of page
